...
- Session 27: Re-introduced `AuiDisplay.INLINE` (chat-flow) alongside `EXPANDED` (focused, host-routable). Library renders both identically; hosts route EXPANDED to a card stub → ModalBottomSheet (narrow, < 600dp) or side detail pane (wide, ≥ 600dp). Added optional `card_title`/`card_description` to `AuiResponse`. Demo: `ExpandedResponseCard` stub, `BoxWithConstraints`-based two-pane LiveChatScreen with sticky-latest detail pane, per-turn `contextHints` (device width/height/layout) appended to the system prompt. Updated FakeLlmClient (5 scripted responses) + all-blocks-showcase JSON. Spec, catalog prompt, and architecture doc updated. 8 new tests.
- Session 28: Rebrand `sheet` → `survey` and simplify. Wire format: `display: "survey"`, `survey_title`, steps with only `question` + `blocks` (dropped `label`/`skippable`/`optional`/`sheet_dismissable`). AI no longer authors `button_primary` or `submit` inside steps — the library injects Back/Next/Submit in the new `SurveyFlowDisplay`. Users can submit any subset; unanswered steps are excluded from `formattedEntries`/`entries`. Removed `TERMINAL_SHEET_ACTIONS` / `isTerminalSheetAction`. Renamed tests + spec examples (`poll-sheet-*` → `poll-survey-*`). Survey now requires ≥2 steps; single-form examples (Article, Issue) were reshaped (expanded or multi-step survey). Spec, architecture, README, catalog prompt + tests all updated.
- Docs sync (2026-04-17): Updated README, `docs/architecture.md`, and `spec/aui-spec-v1.md` to match current code — 18 built-in components (not 50+), library renders INLINE and EXPANDED identically, surveys emit flat content (no built-in sheet — hosts own the container), added coverage of `AuiResponseCard` + `card_title` / `card_description`.
- Status variants (2026-04-17): Filled out the status family — added `badge_info`/`badge_warning`/`badge_error` and `status_banner_info`/`status_banner_warning`/`status_banner_error`. `AuiColors` gained info/warning/error quads (Default palette + `fromColorScheme()` mapping). Error banner reuses `Icons.Default.Warning` since `Error` lives only in material-icons-extended. Built-in component count 18 → 24. Spec doc at `.planning/session-status-variants.md` (source of truth for iOS port). 2 new parser tests, catalog prompt + showcase JSON updated.
- Next: Session 29 — OpenAiLlmClient. See `.planning/phase5-live-chat.md`.
