            ...
            if (isTwoPane) {
                Row(modifier = Modifier.fillMaxSize()) {
                    ChatList(
                        ...
                        renderStateForMessage = viewModel::renderStateFor,
                        modifier = Modifier
                            .weight(1f)
                            .fillMaxHeight(),
                    )
...
private fun ChatList(
    ...
    renderStateForMessage: (String) -> AuiRenderState,
        ...
        items(items = messages, key = { it.id }) { message ->
            when (message.role) {
                ...
                ChatMessage.Role.ASSISTANT -> AssistantMessage(
                    ...
                    renderStateForMessage = renderStateForMessage,
                )
            }
        }
        if (isSending) {
...
private fun DetailPane(
    ...
    Box(modifier = modifier) {
        ...
        Column(
            ...
            AuiRenderer(
                response = response,
                state = renderStateForMessage(message.id),
                modifier = Modifier
                    .fillMaxWidth()
                    .padding(top = 16.dp),
                theme = auiTheme,
